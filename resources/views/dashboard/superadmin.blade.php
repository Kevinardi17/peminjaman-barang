<x-dashboard-layout title="Dashboard Superadmin">
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div class="bg-white rounded-2xl shadow-sm p-5 border">
            <p class="text-sm text-slate-500">Total Jurusan</p>
            <h3 class="text-3xl font-bold mt-2">{{ $totalJurusan }}</h3>
        </div>

        <div class="bg-white rounded-2xl shadow-sm p-5 border">
            <p class="text-sm text-slate-500">Total User</p>
            <h3 class="text-3xl font-bold mt-2">{{ $totalUser }}</h3>
        </div>

        <div class="bg-white rounded-2xl shadow-sm p-5 border">
            <p class="text-sm text-slate-500">Total Barang</p>
            <h3 class="text-3xl font-bold mt-2">{{ $totalBarang }}</h3>
        </div>

        <div class="bg-white rounded-2xl shadow-sm p-5 border">
            <p class="text-sm text-slate-500">Peminjaman Aktif</p>
            <h3 class="text-3xl font-bold mt-2">{{ $totalPeminjamanAktif }}</h3>
        </div>
    </div>
</x-dashboard-layout>